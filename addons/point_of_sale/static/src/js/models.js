function openerp_pos_models(instance, module){ //module is instance.point_of_sale
    ...
    module.PosModel = Backbone.Model.extend({
        ...
        {
            model:  'res.users',
            fields: ['name','company_id','udiscount','password'],
            domain: function(self){ return [['id','=',self.session.uid]]; },
            loaded: function(self,users){ self.user = users[0]; },
        },{
            model:  'pos.password',
            fields: ['id','password', 'udiscount'],
	    domain: null,
            loaded: function(self, pass){
		self.pass2=pass;
                self.db.add_pass(pass);
            },
        },{ 
            model:  'res.company',
    ...
    module.Orderline = Backbone.Model.extend({
	   ...
	   dsc= this.pos.user.udiscount;

if(parseFloat(discount)>parseFloat(dsc)){
if(this.pos.get('selectedOrder').get_clave()){
pass=this.pos.get('selectedOrder').get_clave();
}else{
pass=prompt("Desea Ingresar la Clave para admitir un Descuento Mayor");
passes=(this.pos.pass2);

}
for(var i = 0, len = passes.length; i<len;i++){
if (pass==passes[i].password){
newdiscount=passes[i].udiscount;
this.pos.get('selectedOrder').set_clave(pass)
if (newdiscount){
if (newdiscount<=passes[i].udiscount){
dsc=newdiscount;
}}}
            ...
            var disc = Math.min(Math.max(parseFloat(discount) || 0, 0),dsc);
            this.discount = disc;
	...
	set_clave: function(client) {
            this.set('clave',client);
        },
	get_clave: function() {
            return this.get('clave');

        },
	set_pass: function(client) {
            this.set('pass',client);
        },
	get_pass: function() {
